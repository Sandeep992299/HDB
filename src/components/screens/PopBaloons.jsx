...
export default function BalloonPage() {
    ...
    return (
        <div className="flex flex-col items-center justify-between min-h-screen bg-[#fff0f3] p-6 overflow-hidden">
            ...
            <div className="flex flex-wrap justify-center gap-8 md:gap-16 w-full max-w-4xl">
                {BALLOONS.map((b, index) => (
                    <div key={b.id} className="relative flex flex-col items-center">
                        ...
                        <div className={`absolute top-10 transition-all duration-500 text-4xl md:text-6xl font-black text-[#ff4d6d] ${popped.includes(b.id) ? "opacity-100 scale-125" : "opacity-0 scale-50"}`}>
                            {b.word}
                        </div>
                        ...
                        {!popped.includes(b.id) && (
                            ...
                            >
                                {/* Balloon String */}
                                <div className="absolute -bottom-16 left-1/2 w-0.5 h-16 bg-gray-400"></div>
                                {/* Shine Effect */}
            ...
            {/* Final Message */}
            <div className={`transition-all duration-1000 mb-20 text-center ${allPopped ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
                <h1 className="text-5xl md:text-7xl font-extrabold text-[#ff4d6d] drop-shadow-lg">
                    Forever Yours ❤️
                </h1>
                <p className="text-pink-400 text-xl mt-4 italic font-medium">You are the best thing that ever happened to me.</p>
            </div>
